...
const SpecialProductsClient = ({ products }: { products: Product[] }) => {
  ...
  const handleLikeClick = async (id: number, currentStatus: boolean) => {
    try {
      await toggleProductLike(id, currentStatus);
    } catch (error) {
      console.error("Gagal menyukai produk", error);
    }
  };
    ...
    <>
      {products.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
          ...
          <p className="text-lg font-medium">Tidak ada produk spesial saat ini.</p>
        </div>
      ) : (
...
export default SpecialProductsClient;